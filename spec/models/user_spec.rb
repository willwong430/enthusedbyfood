...
describe User do
  ...
  before do
    @user = User.new(name: "The Dude", email: "thedude@abides.org",
  ...
  describe "activity associations" do
    ...
    describe "status" do
      ...
      before do
        ...
        3.times { followed_user.activities.create!(content: "Lorem ipusm") }
      end
      
      its(:tree) { should include(newer_activity) }
      its(:tree) { should include(older_activity) }
      its(:tree) { should_not include(unfollowed_post) }
      its(:tree) do
